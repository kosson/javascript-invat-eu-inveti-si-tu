...
În momentul în care deschizi browserul, în spatele interfeței grafice este creat ceea ce se numește **mediul de execuție**. Tot software-ul care rulează în browser este limitat în acțiune la granițele browserului. Vorbim despre un mediu separat numit *mediul de execuție* (în engleză *Execution environment*). Poți să-ți imaginezi un singur proces care pornește și inițializează totul.

**Moment Zen**: JavaScript are un singur fir de execuție și o unică stivă pentru gestionarea apelurilor (*call stack*).

De multe ori veți auzi că JavaScript are un singur fir de execuție (*thread*), dar întrebarea este ce este acest *fir*. Puteți să vi-l imaginați ca pe un fir roșu care pornește de la prima linie de cod a programului în contextul mediului global și parcurge toate expresiile care trebuie evaluate. Ați putea să asemuiți parcursul firului de execuție cu firul de tricotat, care în funcție de mișcarea andrelelor, de modelul tricotajului, se aranjează/constituie produsul final. Din contextul global, firul de execuție intră în funcții, în bucle, iese din funcții, intră în blocuri de decizie, ș.a.m.d. Este similar unui fir de tricotaj. În acest context o referință la mașina lui Jacquard este cu atât mai lămuritoare pentru ce este acest fir de execuție.
...
![O descriere vizuală a buclei evenimentelor](img/EventLoop.png)
...
Cât privește codul JavaScript și execuția sa de către interpretor, există un concept fundamental necesar înțelegerii în adâncime. Acesta este cel de **control**. Vom spune că o anumită funcție este executată de **control** sau că acesta este returnat unei funcții care era în așteptare în stivă. **Controlul** este firul roșu al execuției fragmentelor de cod ca răspuns la anumite evenimente.

Sunt convins că te-ai întrebat de foarte multe ori cum *știe* browserul să reacționeze la un click, de exemplu. Ori de câte ori apare un **eveniment** în browser, acesta este transformat într-o **sarcină** (execuția unei funcții cu rol de callback), care trebuie îndeplinită. Adu-ți aminte că o funcție poate fi pasată unei alteia ca valoare a unui argument, pentru a fi executată la un moment dat. Callback-urile reprezintă modul în care comunicăm intențiile noastre unui API. Codul din callback ne aparține și este cel care va face ceva cu rezultatul pe care-l va primi de la API-ul căruia i-l pasăm. Callback-ul va fi programat spre execuție de îndată ce API-ul a adus niște rezultate. Ceea ce se petrece este că toate aceste API-uri, până să execute callback-ul, au nevoie de timp să-și facă și ele treaba: să aducă resurse de pe net, să facă computații care necesită putere de calcul, etc. Aceasta este natura asincronă a JavaScript-ului.
...
![Ordinea în care sunt executate diferitele funcții după rolul lor](img/OrdineaDeExecutieDupaTip.png)
